# -*- coding: utf-8 -*-
"""
Symmetric pocket pattern generator for leatherworking.

Generates two-panel design with symmetric hole alignment for 'meat to meat' sewing.
Ensures both endpoints of seam path have holes and hole count is even for perfect pairing.
...
- R=10mm corner radius and finger notch (user preference)
...
OUT = "/root/leather_sleeve/symmetric"
...
CORNER_RADIUS = 1.0           # 10mm radius for corners and finger notch
NOTCH_DEPTH = 1.0             # 10mm finger notch depth
...
def generate_seam_path():
    ...
    # Finger notch (semi-circle)
    theta = np.linspace(np.pi/2, -np.pi/2, 20)
    for t in theta:
        x = PANEL_W - HOLE_FROM_EDGE - CORNER_RADIUS * np.cos(t)
        y = NOTCH_CENTER_Y + CORNER_RADIUS * np.sin(t)
        path.append((x, y))
...
def generate_pattern(page_size="A2"):
    ...
    # Save
    filename = f"{OUT}/symmetric-pocket-{page_size.lower()}.pdf"
...
if __name__ == "__main__":
    ...
    print("- R=10mm corner radius and finger notch")
