...
#
# _vendor/* submodules only store a pointer (gitlink + SHA). This script copies the
# real files (minus nested .git) into _vendored_snapshot/<name>/, which IS committed to
# the superproject. Agents do NOT read from here — they use skills/* (symlinks into
# _vendor). This dir is restore-only.
#
...
# name|path-inside-_vendor  (one per line; edit when adding a skill)
...
echo "Commit & push (via feature branch + PR):  git add -A && git commit -m \"snapshot: $(date +%F)\""
