...
**`HERMES_HOME` is MANDATORY and must be a throwaway dir — never omit it.**
`HERMES_HOME` is where Hermes keeps `config.yaml`, provider API keys, and the
entire chat-session database (`state.db`). If you omit it, the instance falls
back to the default `~/.hermes` — the *user's real install* — and will:

- read their real `config.yaml` + API keys (so it talks to their real paid LLM
  providers);
- **write your test messages into their real session history** (`state.db`
  / `state.db-wal`), polluting their actual chat log with throwaway chatter.

This happened once: a debug instance launched without `HERMES_HOME` sent a test
prompt to the real model and the reply landed in the user's `~/.hermes/state.db`
as a new session. Recover by deleting just that test session from the `sessions`
/`messages` tables by its `session_id` (see `references/isolated-instance.md`).
...
> `HERMES_DESKTOP_USER_DATA_DIR` is NOT a substitute for `HERMES_HOME`. The former
> is only the Chromium profile (window/cache); the latter is the data dir with
> config, keys, and chats. Setting one and not the other still contaminates.

`npm run perf:serve` does the same with a temp `HERMES_HOME` baked in, if you
also want the perf harness. For a full editable + mock-backed + CDP-open recipe
(including the Python venv so the composer isn't disabled), see
`references/isolated-instance.md`.
