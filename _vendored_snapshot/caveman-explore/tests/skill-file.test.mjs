...
test('frontmatter name matches directory and declares read-only cheap explorer', () => {
  ...
  assert.match(
    fm,
    /^tools:\s*Read,\s*Glob,\s*Grep\s*$/m,
    'tools must be exactly three read-only tools',
  );
  assert.doesNotMatch(
    fm,
    /\b(Edit|Write|Bash|NotebookEdit)\b/,
    'explorer must not have write or execution tools',
  );
...
test('description says when to invoke and skip', () => {
  ...
  assert.match(
    fm,
    /cold-start|cross-file|localization|search has failed/i,
    'must say when to invoke',
  );
...
test('artifact carries no placeholder markers', () => {
  ...
  for (const marker of banned) {
    assert.doesNotMatch(
      ...
      `artifact must not contain ${marker}`,
    );
  }
});
